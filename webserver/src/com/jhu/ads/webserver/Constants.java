package com.jhu.ads.webserver;

public interface Constants {
    
    public static String MAIN_TAG = "DataCentersConfig";
    
    public static String DATA_CENTER_LIST_TAG = "DataCenterList";
    public static String DATA_CENTER_TAG = "DataCenter";
    public static String DATA_CENTER_NAME_TAG = "Name";
    public static String DATA_CENTER_IP_TAG = "Latitude";
    public static String DATA_CENTER_LONGITUDE_TAG = "Longitude";
    public static String DATA_CENTER_LATITUDE_TAG = "Latitude";
    public static String DATA_CENTER_SP_GROUPNAME_TAG = "SpreadGroupName";
    
    
    public static String ZIPCODE_DC_MAP_TAG = "ZipcodeDataCenterMap";
    public static String KEY_VALUE_PAIR_TAG = "KeyValuePair";
    public static String ZIPCODE_TAG = "ZipCode";
    public static String DATA_CENTER_ORDER_TAG = "DataCenterOrder";

}
